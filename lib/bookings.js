...
export function sanitizeBookingInput(body, options = {}) {
  ...
  const firstName = normalizeGuestName(body.firstName);
  const lastName = normalizeGuestName(body.lastName);
  const email = normalizeGuestEmail(body.email);
  const phone = normalizeGuestPhone(body.phone);
  const notes = normalizeGuestNotes(body.notes);
...
function normalizeGuestNotes(value) {
