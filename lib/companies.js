...
export async function listCompaniesTree() {
  const [companiesResult, establishmentsResult, seatCountsResult, openingHoursResult, widgetThemesResult] = await Promise.all([
    ...
    pool.query(
      `SELECT id, establishment_id, seat_count, guest_visit_minutes
...
export function sanitizeSeatCountInput(body) {
  ...
  return { establishmentId, seatCount, guestVisitMinutes };
}

export async function createSeatCount(input) {
  const result = await pool.query(
    `INSERT INTO establishment_seat_counts (establishment_id, seat_count, guest_visit_minutes)
     VALUES ($1, $2, $3)
     RETURNING id, establishment_id, seat_count, guest_visit_minutes`,
    [input.establishmentId, input.seatCount, input.guestVisitMinutes],
  );
...
export async function updateSeatCount(seatCountId, input) {
  ...
  const result = await pool.query(
    `UPDATE establishment_seat_counts
     ...
     WHERE id = $3
     RETURNING id, establishment_id, seat_count, guest_visit_minutes`,
    [input.seatCount, input.guestVisitMinutes, normalizedSeatCountId],
  );
