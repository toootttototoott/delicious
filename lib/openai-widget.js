...
export async function generateWidgetCssSuggestion({
  ...
  const cssText = normalizeCssOutput(extractOutputText(payload));
  if (!cssText) {
...
export function sanitizeWidgetEditorRequest(body) {
...
function buildInstructions(widgetKey) {
  return [
    ...
    "Calendar day cells expose --seat-load and --seat-load-raw custom properties that may be overridden per state.",
...
function sanitizeAttachments(value) {
  ...
  for (const item of value) {
    ...
    if (totalLength > 3_200_000) {
      return { error: "Attachments are too large for one request. Keep them under roughly 2.5 MB total." };
    }
